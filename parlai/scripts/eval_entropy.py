...
def entropy(clusterid_lst):
    ...
    probs = {clusterid: count/total for clusterid,count in counter.items()}
    entropy = 0.0
...
def eval_model(opt, printargs=None, print_parser=None):
    ...
    # calculate entropy
    assert len(predicted_clusterids) == report['exs']
    assert len(target_clusterids) == report['exs']
    print("The predicted cluster distribution (%i samples) has entropy: %f" % (len(predicted_clusterids), entropy(predicted_clusterids)))
    print("The gold cluster distribution (%i samples) has entropy: %f" % (len(target_clusterids), entropy(target_clusterids)))
