...
def eval_model(opt, printargs=None, print_parser=None):
    ...
    cnt = 0
    with open(opt.get('outfile'), "w") as outfile:
        while not world.epoch_done():
            ...
            for w in world.worlds:
                try:
                    prediction = w.acts[1]['text'] # str
                    target_clusterid = int(w.acts[0]['target_clusterid']) # int
                    outfile.write(str(target_clusterid) + " " + prediction + "\n")
                except:
                    continue
